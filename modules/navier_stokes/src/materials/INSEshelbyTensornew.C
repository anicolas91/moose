...
#include "INSEshelbyTensornew.h"
#include "RankTwoTensor.h"
#include "MooseMesh.h"

registerMooseObject("NavierStokesApp", INSEshelbyTensornew);

InputParameters
INSEshelbyTensornew::validParams()
{
...
INSEshelbyTensornew::INSEshelbyTensornew(const InputParameters & parameters)
                               ...
                               : nullptr)
{
  ...
  if (_has_temp && !_total_deigenstrain_dT)
    mooseError("INSEshelbyTensornew Error: To include thermal strain term in Fracture integral "
...
void
INSEshelbyTensornew::initQpStatefulProperties()
{
}

void
INSEshelbyTensornew::computeQpProperties()
{
  RankTwoTensor F((*_grad_disp[0])[_qp]*2.5e7,
                  (*_grad_disp[1])[_qp]*2.5e7,
                  (*_grad_disp[2])[_qp]*2.5e7); // Deformation gradient
  // F(0,0) = (*_disp[0])[_qp]*1.0e9;
  // F(1,1) = (*_disp[1])[_qp]*1.0e9;
  // F(2,2) = (*_disp[2])[_qp]*1.0e9;
  // Moose::out<<"F : "<<F(0,0)<<"\n";
  RankTwoTensor H(F);
  F.addIa(1.0);
  Real detF = F.det();
  ...
  if (_compute_dissipation == true)
  {
    ...
    // F_dot = (F - F_old)/dt
    RankTwoTensor F_dot((*_grad_disp[0])[_qp]*10.0*3.0/2.0, //16.0
                        (*_grad_disp[1])[_qp]*10.0*3.0/2.0,
                        (*_grad_disp[2])[_qp]*10.0*3.0/2.0);
    // F_dot(0,0) = (*_disp[0])[_qp]*1.0;
    // F_dot(1,1) = (*_disp[1])[_qp]*1.0;
    // F_dot(2,2) = (*_disp[2])[_qp]*1.0;
    // Moose::out<<"Fdot : "<<F_dot(0,0)<<"\n";

    // FdotTP = Fdot^T * P = Fdot^T * detF * sigma * FinvT;
    RankTwoTensor FdotTP = F_dot.transpose() * P;

    (*_eshelby_tensor_dissipation)[_qp] = Wdot - FdotTP;
    // Moose::out<<"Eshelby : "<<Wdot - FdotTP<<"\n";
  }
